...
public class ColumnCombinationBitset implements Comparable<ColumnCombinationBitset>, Serializable {
    ...
    public void setBitset(BitSet obs) {
        this.bitset = obs;
    }
    ...
    /**
     * Sets the given BitSet, the previous state is overwritten!
     *
    ...
    /**
     * Returns true if the potentialSuperset contains all columns of this column
     * combination.
     *
    ...
    /**
     * Returns true iff the potentialProperSuperSet contains all columns of this
     * column combination and the potentialProperSuperSet is not equal to this
     * column combination (e.g. proper superset).
     *
     * @param potentialProperSuperSet that this column combination could be a
     * proper subset of
    ...
    /**
     * Returns true iff the potentialSubset contains no columns that are not in
     * this column combination.
     *
    ...
    /**
     * Returns true iff the potentialProperSubset contains no columns that are
     * not in this column combination and the potentialProperSubset is not equal
     * to this column combination (e.g. real subset).
     *
     * @param potentialProperSubset that this column could be a proper superset
     * of
    ...
    /**
     * Returns all subset column combinations of size n that are superset of the
     * subset (first parameter).
     *
    ...
    /**
     * Returns all the cleared bits within the maximum of all the number of
     * columns.
     *
    ...
    public ColumnCombinationBitset minus(
        ...
        BitSet temporaryBitset = (BitSet) this.bitset.clone();
    ...
    /**
     * Union should return a {@link ColumnCombinationBitset} with all the
     * columns from both column combinations and no other columns. The original
     * {@link ColumnCombinationBitset}s should remain unchanged.
     *
    ...
    public ColumnCombinationBitset union(ColumnCombinationBitset other) {
        BitSet unionBitSet = (BitSet) this.bitset.clone();
        unionBitSet.or(other.bitset);
        return new ColumnCombinationBitset().setColumns(unionBitSet);
    }

    /**
     * Intersect should return a {@link ColumnCombinationBitset} with only the
     * columns that are contained in both combinations. The original
     * {@link ColumnCombinationBitset}s should remain unchanged.
     *
    ...
    public ColumnCombinationBitset intersect(ColumnCombinationBitset other) {
        BitSet intersectionBitSet = (BitSet) this.bitset.clone();
        intersectionBitSet.and(other.bitset);
        return new ColumnCombinationBitset().setColumns(intersectionBitSet);
    }

    /**
     * Generates the direct super sets. Supersets are bounded by the maximum
     * number of columns.
     *
    ...
    /**
     * Generates the direct subset column combinations that are superset of the
     * given sub set.
     *
     ...
     * @return the inverted
     * {@link de.metanome.algorithm_helper.data_structures.ColumnCombinationBitset}
     */
